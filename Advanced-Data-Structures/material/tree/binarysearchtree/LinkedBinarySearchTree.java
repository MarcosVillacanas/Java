...
//    }
    ...
    public Position<E> first() throws RuntimeException {
        if (this.binTree.root().getElement() == null) {
            throw new RuntimeException("No first element.");
        }
        Position<E> p = this.binTree.root();
        while (this.binTree.hasLeft(p) && this.binTree.left(p).getElement() != null) p = this.binTree.left(p);
        return p;
    }
    ...
    public Position<E> last() throws RuntimeException {
        ...
        Position<E> p = this.binTree.root();
        while (this.binTree.hasRight(p) && this.binTree.right(p).getElement() != null) p = this.binTree.right(p);
        return p;
    }

    public Iterable<Position<E>> successors(Position<E> pos) {
        List<Position<E>> suc = new LinkedList<>();
        this.successors(pos, pos, suc);
        return suc;
    }

    private void successors (Position<E> pos, Position<E> ant, List<Position<E>> suc) {

        int comp = this.comparator.compare(pos.getElement(), ant.getElement());

        if (comp < 0 || pos.equals(ant)) {
            suc.add(ant);
            BinaryTree<E> bt = this.binTree.subTree(this.binTree.right(ant));
            bt.iterator().forEachRemaining( p -> { if (p != null && p.getElement() != null) suc.add(p); });
            this.binTree.attachRight(ant, bt);
        }
        else if (comp == 0) suc.add(ant);

        if (!this.binTree.isRoot(ant)) successors(pos, this.binTree.parent(ant), suc);
    }

    public Iterable<Position<E>> predecessors(Position<E> pos){
        return this.predecessors(pos, pos, new LinkedList<>());
    }

    private Iterable<Position<E>> predecessors (Position<E> pos, Position<E> ant, List<Position<E>> pre) {
        int compare = this.comparator.compare(pos.getElement(), ant.getElement());

        if (compare >= 0) {
            pre.add(ant);
            BinaryTree<E> bt = this.binTree.subTree(this.binTree.left(ant));
            bt.iterator().forEachRemaining(p -> { if (p != null && p.getElement() != null) pre.add(p); });
            this.binTree.attachLeft(ant, bt);
        }

        return (this.binTree.isRoot(ant))? pre : predecessors(pos, this.binTree.parent(ant), pre);
    }
    ...
    public Iterable<Position<E>> findRange(E minValue, E maxValue) throws RuntimeException {
        if (this.comparator.compare(minValue, maxValue) > 0) throw new RuntimeException("Invalid range. (min>max");
        List<Position<E>> inRange = new LinkedList<>();
        this.findRange(minValue, maxValue, this.binTree.root(), inRange);
        return inRange;
    }

    private void findRange (E minValue, E maxValue, Position<E> p, List<Position<E>> inRange) {

        if (p == null || p.getElement() == null) return;

        int comp1 = this.comparator.compare(minValue, p.getElement());
        int comp2 = this.comparator.compare(p.getElement(), maxValue);

        if (comp1 <= 0) this.findRange(minValue, maxValue, this.binTree.left(p), inRange);
        if (comp1 <= 0 && comp2 <= 0) inRange.add(p);
        if (comp2 <= 0) this.findRange(minValue, maxValue, this.binTree.right(p), inRange);
    }

    public Position<E> successor (Position<E> p) throws RuntimeException {
        if (this.binTree.hasRight(p)) {
            Position<E> left = this.binTree.right(p);
            while (this.binTree.hasLeft(left) && this.binTree.left(left).getElement() != null)
                left = this.binTree.left(left);
            return left;
        }
        else {
            Position<E> ant = p;
            while (!this.binTree.isRoot(ant)) {
                Position<E> father = this.binTree.parent(ant);
                if (this.binTree.hasLeft(father) && this.binTree.left(father).equals(ant)) return father;
                ant = father;
            }
    ...
    public Position<E> predecessor (Position<E> p) throws RuntimeException {
        if (this.binTree.hasLeft(p)) {
            Position<E> right = this.binTree.left(p);
            while (this.binTree.hasRight(right) && this.binTree.right(p).getElement() != null)
                right = this.binTree.right(right);
            return right;
        }
        else {
            Position<E> ant = p;
            while (!this.binTree.isRoot(ant)) {
                Position<E> father = this.binTree.parent(ant);
                if (this.binTree.hasRight(father) && this.binTree.right(father).equals(ant)) return father;
                ant = father;
            }
    ...
    public Iterable<Position<E>> fromLowerLimit (E minValue) {
        List<Position<E>> fromLower = new LinkedList<>();
        this.fromLowerLimit(minValue, this.binTree.root(), fromLower);
        return fromLower;
    }

    private void fromLowerLimit (E minValue, Position<E> p, List<Position<E>> fromLower) {
        if (p == null || p.getElement() == null) return;

        int comp = this.comparator.compare(minValue, p.getElement());
        if (comp <= 0) {
            fromLower.add(p);
            this.fromLowerLimit(minValue, this.binTree.left(p), fromLower);
        }
        this.fromLowerLimit(minValue, this.binTree.right(p), fromLower);
    }

    public Iterable<Position<E>> untilUpperLimit (E maxValue) {
        List<Position<E>> untilUpper = new LinkedList<>();
        this.untilUpperLimit(maxValue, this.binTree.root(), untilUpper);
        return untilUpper;
    }

    private void untilUpperLimit (E maxValue, Position<E> p, List<Position<E>> untilUpper) {
        if (p == null || p.getElement() == null) return;

        int comp = this.comparator.compare(p.getElement(), maxValue);
        if (comp <= 0) {
            untilUpper.add(p);
            this.untilUpperLimit(maxValue, this.binTree.right(p), untilUpper);
        }
        this.untilUpperLimit(maxValue, this.binTree.left(p), untilUpper);
    }

    public Iterable<Position<E>> outOfRange (E minValue, E maxValue) throws RuntimeException {
        if (this.comparator.compare(minValue, maxValue) > 0) throw new RuntimeException("Invalid range. (min>max");
        List<Position<E>> outRange = new LinkedList<>();
        this.outOfRange(minValue, maxValue, this.binTree.root(), outRange);
        return outRange;
    }

    private void outOfRange (E minValue, E maxValue, Position<E> p, List<Position<E>> outRange) {
        if (p == null || p.getElement() == null) return;

        int comp1 = this.comparator.compare(minValue, p.getElement());
        int comp2 = this.comparator.compare(p.getElement(), maxValue);

        if (comp1 > 0 || comp2 < 0) outRange.add(p);

        this.outOfRange(minValue, maxValue, this.binTree.left(p), outRange);
        this.outOfRange(minValue, maxValue, this.binTree.right(p), outRange);
    }

    public Iterable<Position<E>> inOrderTraversal () {
        return this.inOrderTraversal(this.binTree.root(), new LinkedList<>());
    }

    private Iterable<Position<E>> inOrderTraversal (Position<E> p, List<Position<E>> inOrderTraversal) {
        if (p == null || p.getElement() == null) return inOrderTraversal;
        this.inOrderTraversal(this.binTree.left(p), inOrderTraversal);
        inOrderTraversal.add(p);
        this.inOrderTraversal(this.binTree.right(p), inOrderTraversal);
        return inOrderTraversal;
    }

    public LinkedTree<E> toLinkedTree () {
        LinkedTree<E> lt = new LinkedTree<>();
        toLinkedTree(this.binTree.root(), lt.addRoot(this.binTree.root().getElement()), lt);
        return lt;
    }

    private void toLinkedTree (Position<E> origin, Position<E> dest, LinkedTree<E> lt) {

        if (origin == null || origin.getElement() == null) return;

        for (Position<E> c : this.binTree.children(origin))
            toLinkedTree(c, lt.add(c.getElement(), dest), lt);
    }

    public void removeRange (E min, E max) { this.findRange(min, max).forEach(this::remove); }
}
